...
class _ProfileScreenState extends State<ProfileScreen> with TickerProviderStateMixin {
  ...
  String _cacheBustUrl(String url) {
    try {
      final String absoluteUrl = url.startsWith('/') ? '${AuthService.baseUrl}$url' : url;
  ...
  Widget _buildAvatarImage() {
    ...
    if (src.startsWith('/')) {
      final String absolute = '${AuthService.baseUrl}$src';
      ...
      return ClipOval(
        ...
        errorBuilder: (context, error, stackTrace) {
          print('❌ Image.network error: $error');
            print('💡 SOLUTION: Backend should return base64 format');
            return placeholder;
        },
        loadingBuilder: (context, child, loadingProgress) {
          if (loadingProgress == null) {
              // ✅ Chỉ log success 1 lần duy nhất
    ...
    if (src.startsWith('http')) {
      ...
      return ClipOval(
        ...
        errorBuilder: (context, error, stackTrace) {
            ...
            print('   URL: $src');
            print('   Error: $error');
            print('💡 TIP: Ask backend to return base64 format instead');
            return placeholder;
        },
        loadingBuilder: (context, child, loadingProgress) {
            if (loadingProgress == null) {
              ...
              if (!_hasLoggedSuccess) {
                _hasLoggedSuccess = true;
                print('✅ SUCCESS! Avatar loaded from URL');
              }
              return child;
            }
            // ✅ Bỏ log loading progress để giảm spam
            return const Center(
              child: SizedBox(
                width: 24,
                height: 24,
